...
DG390BDJ-E3:                https://www.vishay.com/docs/71404/71404.pdf
...
const int DATA_BUTTON = 3;        // Knapp
const int LED_RED = 4;            // Rød LED
const int LED_GREEN = 5;          // Grønn LED
const int ANALOG_SWITCH = 6;      // Aktiviering av Analog Switch
const int WIRE_BOTTOM_READ = A0;  // Kontrollkurs for undersiden av piezoen
const int WIRE_TOP_READ = A1;     // Kontrollkurs for oversiden av piezoen
...
*/
  ...
  for (int i = 2; i <= 19; i++) {  // Setter alle digitale og analoge pinnenr til INPUT, slik at de er i en kjent tilstand
    pinMode(i, INPUT_PULLUP);
  }

  pinMode(PULSE_PIN, INPUT);           // Setter pulspinnen fra 555 til INPUT
  pinMode(ANALOG_SWITCH, OUTPUT);      // Setter pinnen til den analoge bryteren til OUTPUT
  pinMode(DATA_BUTTON, INPUT_PULLUP);  // Setter pinnen til dataknappen til INPUT_PULLUP

  pinMode(WIRE_BOTTOM_READ, INPUT_PULLUP);  // Setter pinnen som skal sjekke ledningen til bunnsiden av piezoen til INPUT_PULLUP
  pinMode(WIRE_TOP_READ, INPUT);            // Setter pinnen som skal sjekke ledningen til oversiden av piezoen til INPUT

  pinMode(LED_RED, OUTPUT);    // Rød LED
  pinMode(LED_GREEN, OUTPUT);  // Grønn LED
  ...
  display.drawBitmap(0, 0, FiVLogo, SCREEN_WIDTH, SCREEN_HEIGHT, 1);
  display.display();
  ...
  display.println("Piezotester-Wired.ino\n");
...
void loop() {
  const unsigned long outsideDisplayBounds = 76;
  ...
  if (measuredCapacitance >= lowerCapacitanceLimit && measuredCapacitance <= upperCapacitanceLimit) {
    ...
    display.println();
    capFail = false;
  } else if (measuredCapacitance >= upperCapacitanceLimit) {
    display.println("FAIL");
    display.setFont();
    display.print("Capacitance too high");
    display.setCursor(0, outsideDisplayBounds);  // Ganske frekk måte å bli kvitt tekst på uten å slette/flytte all data
    capFail = true;
  } else if (measuredCapacitance <= lowerCapacitanceLimit) {
    display.println("FAIL");
    display.setFont();
    display.println("Capacitane too low\nor no piezo detected");
    display.setCursor(0, outsideDisplayBounds);  // Ganske frekk måte å bli kvitt tekst på uten å slette/flytte all data
    capFail = true;
  }
...
bool wireBottomCheck() {
  //Serial.println(analogRead(WIRE_BOTTOM_READ));    // Debug
  const int bottomWireADCThreshold = 100;

  if (analogRead(WIRE_BOTTOM_READ) <= bottomWireADCThreshold) {
    return true;  // Ledning OK
  } else {
    return false;  // Ledning ikke OK
  }
}


// Følgende funksjon veksler den analoge switchen: DG390BDJ-E3, slik at 555-timeren er utkoblet
// og kursen blir redirigert for å måles. ~ 5V tilsier at ledningen er OK
bool wireTopCheck() {
  //Serial.println(analogRead(WIRE_TOP_READ));   // Debug
  const int topWireADCThreshold = 900;

  digitalWrite(ANALOG_SWITCH, HIGH);  // Veksler DG390BDJ-E3 på
  delayMicroseconds(1);               // Venter på veksling.
  if (analogRead(WIRE_TOP_READ) >= topWireADCThreshold) {
    digitalWrite(ANALOG_SWITCH, LOW);
    delay(1);     // Venter på veksling.
    return true;  // Ledning OK
  } else {
    digitalWrite(ANALOG_SWITCH, LOW);  // Veksler DG390BDJ-E3 av
    delay(1);                          // Venter på veksling.
    return false;                      // Ledning ikke OK
  }
}
